...
if [ $1 -eq 1 ]; then
  echo "------------------------"
  echo "cacher-whonix is being installed"
  echo "------------------------"
  qubesctl state.apply cacher-whonix.create
  qubesctl --skip-dom0 --targets=whonix-gw-16 state.apply cacher-whonix.install
  qubesctl --skip-dom0 --targets=cacher state.apply cacher-whonix.configure
  qubesctl state.apply cacher-whonix.use
  qubesctl --skip-dom0 --templates state.apply cacher-whonix.change_templates
fi

%preun
if [ $1 -eq 0 ]; then
  qubesctl --skip-dom0 --templates state.apply cacher-whonix.restore_templates
fi
